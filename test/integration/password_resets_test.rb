...
class PasswordResetsTest < ActionDispatch::IntegrationTest
  ...
  test "password resets" do
    ...
    assert_template 'password_rests/new'
    # メールアドレスが有効
    post password_rests_path,
         params: { password_reset: { email: @user.email } }
    assert_not_equal @user.reset_digest, @user.reload.reset_digest  # パスワードリセット後のダイジェストが異なるか→異なれば変更できた
    ...
    assert_nil user.reload.reset_digest # ログインしたらダイジェストがnilになっているかどうか確認
    assert is_logged_in?
    assert_not flash.empty?
